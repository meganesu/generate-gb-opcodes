...
const generate_OR_r8 = () => { // eslint-disable-line camelcase
  const instructions = [];
  ...
  Object.entries(registerBinaryCodes)
    .forEach(([register, registerBinaryCode]) => {
      ...
      const opCodeInBinary = `10110${registerBinaryCode}`;
      instruction.opCode = convertBinaryStringToHexString(opCodeInBinary);

      instruction.description = (
        <p>
          {
            `Take the logical OR for each bit of the contents of register ${register}`
...
export default generate_OR_r8; // eslint-disable-line camelcase
